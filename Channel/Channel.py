  ...
  #  Constructor de la clase
   # @param <str> contact_ip: Si no se trabaja de manera local
    ...
    def send_text(self, text, local):
        if(local):
            threading.Thread.__init__(self)
            miMensaje = MyApiClient("localhost:"+str(self.contact_port), text, "texto")
        else:
            threading.Thread.__init__(self)
            a = str(self.contact_port)+ ":5000"
            miMensaje = MyApiClient(a, text, "texto")
     #   """**************************************************
    ...
    def send_video(self, local):

        if(local):
            threading.Thread.__init__(self)
            graba1 = recordVideo(self.contact_port)
            string = "localhost:"+str(self.contact_port)
            graba1.inicia(True, string)
